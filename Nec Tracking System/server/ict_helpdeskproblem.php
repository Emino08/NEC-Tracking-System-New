...
$sql = "SELECT request, username, request_type, request_id, date_time, status,solution FROM request ORDER BY date_time DESC";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        if($row1 = $result1->fetch_assoc()){
            ...
            if(strcmp($row['status'],'1')===0){
              ...
              <td class='w3-center color'> <span class='dot w3-left' style='margin-top:1%'></span>".$row["request_type"]."</td>
              ...
              <div class='w3-third'>
              <textarea maxlength='1000' id='"."text_".$row["request_id"]."' disabled class='w3-white w3-input w3-border w3-round-xxlarge w3-padding'  placeholder='Write a Solution'>".$row["solution"]."</textarea>
              </div>
              <div class='w3-third w3-center'>
              <select id='"."select_".$row["request_id"]."' disabled class='w3-select w3-border w3-round-xxlarge w3-margin' style='height: 40px;width: 70%'  name='option'>
              
                        <option value='1' selected>Solved</option>
                        <option value='2' >Not Solved</option>
                      </select>
                      
              </div>
              <div class='w3-third w3-center'>
              <p><button onclick='save_solution(event)' disabled id='"."button_".$row["request_id"]."' style='margin-bottom: 30px; height: 40px;width: 70%' class='w3-button w3-black w3-round-xxlarge '>Save Solution</button></p>
              </div>
              </div>" .
                          "</p>
                  </footer>
            ...
            echo "<tr onclick=\"document.getElementById('" .$row["request_id"]. "').style.display='block'\">
            <td class='w3-center color'> <span class='dot1 w3-left' style='margin-top:1%'></span>".$row["request_type"]."</td>
            <td class='w3-center color'>".$row["date_time"]."</td>
            </tr>"."
            <td class='color'>
            <div class='w3-container'>
      ...
      <div class='w3-third w3-center'>
        <select id='"."select_".$row["request_id"]."' class='w3-select w3-border w3-round-xxlarge w3-margin' style='height: 40px;width: 70%'  name='option'>
                      <option value='' selected>Problem Status</option>
      ...
      <div class='w3-third w3-center'>
        <p><button onclick='save_solution(event)' id='"."button_".$row["request_id"]."' style='margin-bottom: 30px; height: 40px;width: 70%' class='w3-button w3-black w3-round-xxlarge '>Save Solution</button></p>
              ...
              echo "<tr onclick=\"document.getElementById('" .$row["request_id"]. "').style.display='block'\">
              <td class='w3-center color'> <span class='dot2 w3-left' style='margin-top:1%'></span>".$row["request_type"]."</td>
              <td class='w3-center color'>".$row["date_time"]."</td>
              </tr>"."
              <td class='color'>
              <div class='w3-container'>
